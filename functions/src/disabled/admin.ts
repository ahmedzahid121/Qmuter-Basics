...
router.get('/stats', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const thisMonthUsers = usersSnapshot.docs.filter(doc => {
      ...
      return userData.createdAt?.toDate() >= thisMonth;
    }).length;

    const lastMonthUsers = usersSnapshot.docs.filter(doc => {
      const userData = doc.data();
      return userData.createdAt?.toDate() >= lastMonth && userData.createdAt?.toDate() < thisMonth;
    }).length;
    ...
    const lastMonthRevenue = transactions
      .filter(t => t.status === 'completed' && t.type === 'ride_payment' && 
        t.createdAt?.toDate() >= lastMonth && t.createdAt?.toDate() < thisMonth)
      .reduce((sum, t) => sum + (t.amount || 0), 0);
...
router.get('/routes', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
    const routesSnapshot = await db.collection('routes').get();
    const routes = await Promise.all(routesSnapshot.docs.map(async (doc) => {
      ...
      try {
        ...
        if (driverDoc.exists) {
          ...
          driverName = driverData?.displayName || driverData?.email || 'Unknown';
        }
      } catch (error) {
...
// Get all transactions
router.get('/transactions', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
    const transactionsSnapshot = await db.collection('transactions')
...
// Get system alerts
router.get('/alerts', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
...
// Get analytics data
router.get('/analytics', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
    const now = new Date();
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const ridesSnapshot = await db.collection('transactions')
    ...
    // Get top passengers
    const passengersSnapshot = await db.collection('users')
...
router.post('/users/:userId/suspend', authenticateUser, requireAdmin, validateRequest(suspendUserSchema, 'body'), async (req: Request, res: Response) => {
  try {
...
// Approve route
const approveRouteSchema = Joi.object({
  notes: Joi.string().optional()
});

router.post('/routes/:routeId/approve', authenticateUser, requireAdmin, validateRequest(approveRouteSchema, 'body'), async (req: Request, res: Response) => {
  try {
...
// Reject route
const rejectRouteSchema = Joi.object({
  reason: Joi.string().required(),
  notes: Joi.string().optional()
});

router.post('/routes/:routeId/reject', authenticateUser, requireAdmin, validateRequest(rejectRouteSchema, 'body'), async (req: Request, res: Response) => {
  try {
    const { routeId } = req.params;
    const { reason, notes } = req.body;
...
// Mark alert as read
router.post('/alerts/:alertId/read', authenticateUser, requireAdmin, async (req: Request, res: Response) => {
  try {
